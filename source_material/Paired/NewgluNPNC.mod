...
NEURON {
	...
	RANGE on_mglu
	RANGE F, f, tauF, D1a, d1a, tauD1a, D2a, d2a, tauD2a
	RANGE D1n, d1n, tauD1n, D2n, d2n, tauD2n
	RANGE facfactora,facfactorn
...
PARAMETER {
	...
	: the (1) is needed for the range limits to be effective
        ...
        tauF = 1 (ms) < 1e-9, 1e9 >
        d1a = 0.5 (1) < 0, 1 >     : fast depression
        tauD1a = 25 (ms) < 1e-9, 1e9 >
        d2a = 0.5 (1) < 0, 1 >     : slow depression
        tauD2a = 20 (ms) < 1e-9, 1e9 >		
		d1n = 0.92 (1) < 0, 1 >     : fast depression
        tauD1n = 25 (ms) < 1e-9, 1e9 >
        d2n = 0.85 (1) < 0, 1 >     : slow depression
        tauD2n = 20 (ms) < 1e-9, 1e9 >	
}
...
if (t0>0) {
	...
	g_nmda = gbar_nmda*r_nmda*facfactorn
	inmda = W_nmda*g_nmda*(v - Erev_nmda)*s(v)

	g_ampa = gbar_ampa*r_ampa*facfactora
...
:mGluR5 Ca2+ pool
	...
	wn' = 0  :eta(can)*(L1*omegaN(can) - L3*(w-initW))  :- L4*(wm-initW/2)) 
	wm' = 0   :eta(cam/4)*(L2*omegaM(cam) - L3*(w-initW))  :- L5*(wn-initW/2)) 
	...
	if (t>= 30000 && t<=30000+dt  ) { 
	wm = wm*0.8
	wn = wn*0.8
	}
	if (t >= 80000 && t<=80000+dt) {
	wm = wm*1.25
	wn = wn*1.25
	}
...
NET_RECEIVE(weight (uS)) {
	...
	D1a = 1 - (1-D1a)*exp(-(t - tsyn)/tauD1a)
	D2a = 1 - (1-D2a)*exp(-(t - tsyn)/tauD2a)
	D1n = 1 - (1-D1n)*exp(-(t - tsyn)/tauD1n)
	D2n = 1 - (1-D2n)*exp(-(t - tsyn)/tauD2n)
	tsyn = t
	
	:facfactora = F * D1a * D2a
	:facfactorn = F * D1n * D2n
	:F = F+f  :F * f
