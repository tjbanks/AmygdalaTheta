...
def ecp_psd(ecp,skip_n=0,downsample=20,nfft=1024,fs=1000,noverlap=0,ax=None):
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.plot(f, pxx*10,linewidth=0.6)
    ax.set_ylim([10**-10,.1]) 
...
def run(show_plots=False,save_plots=False):
    ...
    spikes_df = pd.read_csv(spikes_location, sep='\t',header=0,names=['timestamps','node_ids'])
    print("done")

    if show_plots or save_plots:
        ecp_location = 'LFPs/LFP_elec_combine'
        print("loading " + ecp_location)
        #ecp_channel = 0
        #f = h5py.File(ecp_h5_location)
        #data_raw = np.array(f['ecp']['data'])
        #ecp = data_raw.T[ecp_channel] #flip verts and grab channel 0
        ecp = np.loadtxt(ecp_location)
    ...
    if show_plots or save_plots:
        ...
        ecp_psd(ecp, downsample=int(steps_per_ms), skip_n=skip_n, ax=ax2)
